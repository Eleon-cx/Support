...
export default function RecapNextSteps() {
  const recapPoints = [
    "Created effective milestones for project tracking",
    "Mastered handling task dependencies",
    "Learned to set realistic deadlines",
    "Practiced tracking project statuses effectively",
  ]

  const router = useRouter()
  const { areActivitiesCompleted } = useActivities()

  const requiredActivities = ["module4_miniQuiz"]
  ...
  const handleNextModule = () => {
    if (canProceedToNextModule) {
      ...
      const element = document.getElementById("module4_miniQuiz")
      if (element) {
  ...
  return (
    <section className="py-16 px-6 bg-gradient-to-br from-indigo-50 to-blue-100">
      <h2 className="text-4xl font-bold mb-10 text-center text-indigo-700">Module 4 Recap & What's Next</h2>
        ...
        >
          ...
          <div className="bg-indigo-50 p-6 rounded-lg">
            ...
            <p className="text-gray-600 mb-4">
              Experience a full project lifecycle in a simulated environment to apply your project management skills.
            </p>
            <motion.button
